...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var btnRegisterWifi: com.google.android.material.button.MaterialButton
    private lateinit var tvSafeWifisTitle: TextView
    private lateinit var cgSafeWifis: com.google.android.material.chip.ChipGroup
    
    private lateinit var cardOwner: MaterialCardView
    private lateinit var ivOwnerFace: ImageView
    private lateinit var tvOwnerTitle: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // (Removing the duplicate block here)
    ...
    private fun scanNetworkSecurity() {
        ...
        val sharedPrefs = getSharedPreferences("AppLockPrefs", Context.MODE_PRIVATE)
        val registeredWifis = sharedPrefs.getStringSet("SafeWifis", emptySet()) ?: emptySet()
        ...
        if (capabilities != null && capabilities.hasTransport(NetworkCapabilities.TRANSPORT_WIFI)) {
            ...
            if (wifiInfo != null) {
                ...
                val isRegistered = registeredWifis.contains(currentSsid)
                
                var score = 0
                val details = StringBuilder()

                // 1. Registration Check (Highest Priority)
                if (isRegistered) {
                    score = 100
                    details.append(getString(R.string.safe_network))
                    btnRegisterWifi.visibility = View.GONE
                } else {
                    btnRegisterWifi.visibility = View.VISIBLE
                    btnRegisterWifi.setOnClickListener {
                        registerCurrentWifi(currentSsid)
                    }
                    ...
                    score += (level / 4)
                    details.append(getString(R.string.unregistered_wifi))
                }
    ...
    private fun registerCurrentWifi(ssid: String) {
        val sharedPrefs = getSharedPreferences("AppLockPrefs", Context.MODE_PRIVATE)
        val registeredWifis = sharedPrefs.getStringSet("SafeWifis", emptySet())?.toMutableSet() ?: mutableSetOf()
        registeredWifis.add(ssid)
        sharedPrefs.edit().putStringSet("SafeWifis", registeredWifis).apply()
        Toast.makeText(this, getString(R.string.wifi_registered), Toast.LENGTH_SHORT).show()
        scanNetworkSecurity()
        updateSafeWifisUI()
    }

    private fun updateSafeWifisUI() {
        val sharedPrefs = getSharedPreferences("AppLockPrefs", Context.MODE_PRIVATE)
        val registeredWifis = sharedPrefs.getStringSet("SafeWifis", emptySet()) ?: emptySet()

        if (registeredWifis.isNotEmpty()) {
            tvSafeWifisTitle.visibility = View.VISIBLE
            cgSafeWifis.visibility = View.VISIBLE
            cgSafeWifis.removeAllViews()
            
            registeredWifis.forEach { ssid ->
                ...
                cgSafeWifis.addView(chip)
            }
        } else {
            tvSafeWifisTitle.visibility = View.GONE
            cgSafeWifis.visibility = View.GONE
        }
    }

    private fun removeSafeWifi(ssid: String) {
        val sharedPrefs = getSharedPreferences("AppLockPrefs", Context.MODE_PRIVATE)
        val registeredWifis = sharedPrefs.getStringSet("SafeWifis", emptySet())?.toMutableSet() ?: mutableSetOf()
        registeredWifis.remove(ssid)
        sharedPrefs.edit().putStringSet("SafeWifis", registeredWifis).apply()
        updateSafeWifisUI()
        scanNetworkSecurity()
    }
    ...
    private fun setupUsageChart(logs: List<UsageLogEntry>) {
        ...
        val isDarkMode = getSharedPreferences("AppLockPrefs", Context.MODE_PRIVATE).getBoolean("DarkMode", false)
        ...
        usageChart.axisLeft.textColor = textColor
        usageChart.xAxis.textColor = textColor
        usageChart.xAxis.setDrawGridLines(false)
        usageChart.legend.textColor = textColor
    ...
    private fun setupSecurityChart(files: Array<java.io.File>) {
        ...
        securityChart.axisLeft.textColor = textColor
        securityChart.xAxis.textColor = textColor
        securityChart.legend.textColor = textColor
    ...
    private fun updateOwnerUI() {
        val ownerFaceFile = File(getExternalFilesDir(null), "owner_face.jpg")
        if (ownerFaceFile.exists()) {
            val bitmap = BitmapFactory.decodeFile(ownerFaceFile.absolutePath)
            if (bitmap != null) {
                ivOwnerFace.setImageBitmap(bitmap)
                cardOwner.visibility = View.VISIBLE
                tvOwnerTitle.visibility = View.VISIBLE
            } else {
                cardOwner.visibility = View.GONE
                tvOwnerTitle.visibility = View.GONE
            }
        } else {
            cardOwner.visibility = View.GONE
            tvOwnerTitle.visibility = View.GONE
